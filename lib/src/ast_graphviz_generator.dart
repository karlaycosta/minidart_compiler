...
class ASTGraphvizGenerator implements AstVisitor<String> {
  ...
  String generateDot(List<Stmt> statements) {
    ...
    _buffer.writeln(
      '  $rootId [label="📄 Programa MiniDart", fillcolor=lightgreen, shape=ellipse];',
    );
  ...
  void saveAndVisualize(List<Stmt> statements, {String filename = 'ast'}) {
    ...
    // Tentar gerar PNG
    try {
      final result = Process.runSync('dot', [
        '-Tpng',
        '$filename.dot',
        '-o',
        '$filename.png',
      ...
      if (result.exitCode == 0) {
        ...
        print(
          '⚠️  Graphviz não encontrado. Instale para gerar imagens automaticamente.',
        );
  ...
  String visitTypedVarDeclStmt(TypedVarDeclStmt stmt) {
    ...
    _buffer.writeln(
      '  $nodeId [label="📦 $typeName $varName", fillcolor=lightblue];',
    );
  ...
  String visitConstDeclStmt(ConstDeclStmt stmt) {
    ...
    final typeName = _escapeLabel(stmt.type.name);
    _buffer.writeln(
      '  $nodeId [label="🔒 constante $typeName $constName", fillcolor=lightcoral];',
    );
  ...
  String visitIfStmt(IfStmt stmt) {
    final nodeId = _nextId();
    _buffer.writeln(
      '  $nodeId [label="🔀 se", fillcolor=orange, shape=diamond];',
    );
  ...
  String visitWhileStmt(WhileStmt stmt) {
    final nodeId = _nextId();
    _buffer.writeln(
      '  $nodeId [label="🔄 enquanto", fillcolor=orange, shape=diamond];',
    );
  ...
  String visitDoWhileStmt(DoWhileStmt stmt) {
    final nodeId = _nextId();
    _buffer.writeln(
      '  $nodeId [label="🔄 faca-enquanto", fillcolor=orange, shape=diamond];',
    );
  ...
  String visitForStmt(ForStmt stmt) {
    final nodeId = _nextId();
    _buffer.writeln(
      '  $nodeId [label="🔄 para", fillcolor=orange, shape=diamond];',
    );

    // Variável
    _buffer.writeln(
      '  ${nodeId}_var [label="${stmt.variable.lexeme}", fillcolor=lightgreen];',
    );
  ...
  String visitForStepStmt(ForStepStmt stmt) {
    ...
    _buffer.writeln(
      '  $nodeId [label="🔄 para+$direction", fillcolor=orange, shape=diamond];',
    );
  ...
  String visitForCStmt(ForCStmt stmt) {
    final nodeId = _nextId();
    _buffer.writeln(
      '  $nodeId [label="🔄 para(;;)", fillcolor=darkturquoise, shape=diamond];',
    );
  ...
  String visitBinaryExpr(BinaryExpr expr) {
    ...
    _buffer.writeln(
      '  $nodeId [label="⚙️ $op", fillcolor=lightsteelblue, shape=circle];',
    );
  ...
  String visitTernaryExpr(TernaryExpr expr) {
    final nodeId = _nextId();
    _buffer.writeln(
      '  $nodeId [label="❓ ?:", fillcolor=lightyellow, shape=diamond];',
    );
  ...
  String visitLogicalExpr(LogicalExpr expr) {
    ...
    final op = _escapeLabel(expr.operator.lexeme);
    _buffer.writeln(
      '  $nodeId [label="🧠 $op", fillcolor=lightcoral, shape=circle];',
    );
  ...
  String visitLiteralExpr(LiteralExpr expr) {
    ...
    _buffer.writeln(
      '  $nodeId [label="$value", fillcolor=$color, shape=ellipse];',
    );
  ...
  String visitVariableExpr(VariableExpr expr) {
    ...
    _buffer.writeln(
      '  $nodeId [label="🏷️ $name", fillcolor=wheat, shape=ellipse];',
    );
  ...
  String visitGroupingExpr(GroupingExpr expr) {
    final nodeId = _nextId();
    _buffer.writeln(
      '  $nodeId [label="🔘 ( )", fillcolor=lavender, shape=circle];',
    );
  ...
  String visitFunctionStmt(FunctionStmt stmt) {
    ...
    if (stmt.params.isNotEmpty) {
      ...
      _buffer.writeln(
        '  $paramsId [label="📝 parâmetros", fillcolor=lightgreen];',
      );
      ...
      for (int i = 0; i < stmt.params.length; i++) {
        ...
        _buffer.writeln(
          '  $paramId [label="${param.type.name} ${_escapeLabel(param.name.lexeme)}", fillcolor=white];',
        );
  ...
  String visitBreakStmt(BreakStmt stmt) {
    final nodeId = _nextId();
    _buffer.writeln('  $nodeId [label="🛑 parar", fillcolor=red];');
    return nodeId;
  }
  ...
  String visitContinueStmt(ContinueStmt stmt) {
    final nodeId = _nextId();
    _buffer.writeln('  $nodeId [label="⏭️ continuar", fillcolor=yellow];');
    return nodeId;
  }

  @override
  String visitSwitchStmt(SwitchStmt stmt) {
    final nodeId = _nextId();
    _buffer.writeln('  $nodeId [label="🔀 escolha", fillcolor=lightblue];');

    // Conecta à expressão
    final exprId = stmt.expression.accept(this);
    _buffer.writeln('  $nodeId -> $exprId [label="expressão"];');

    // Conecta aos casos
    for (int i = 0; i < stmt.cases.length; i++) {
      final caseStmt = stmt.cases[i];
      final caseId = _nextId();
      
      if (caseStmt.value != null) {
        _buffer.writeln('  $caseId [label="📋 caso", fillcolor=lightyellow];');
        
        // Conecta ao valor do caso
        final valueId = caseStmt.value!.accept(this);
        _buffer.writeln('  $caseId -> $valueId [label="valor"];');
      } else {
        _buffer.writeln('  $caseId [label="📋 contrario", fillcolor=orange];');
      }
      
      _buffer.writeln('  $nodeId -> $caseId [label="caso ${i + 1}"];');
      
      // Conecta aos statements do caso
      for (int j = 0; j < caseStmt.statements.length; j++) {
        final stmtId = caseStmt.statements[j].accept(this);
        _buffer.writeln('  $caseId -> $stmtId [label="stmt ${j + 1}"];');
      }
    }
  ...
  String visitCallExpr(CallExpr expr) {
    final nodeId = _nextId();
    _buffer.writeln(
      '  $nodeId [label="📞 chamada", fillcolor=lightcyan, shape=ellipse];',
    );
    ...
    if (expr.arguments.isNotEmpty) {
      ...
      _buffer.writeln(
        '  $argsId [label="📋 argumentos", fillcolor=lightgreen];',
      );
